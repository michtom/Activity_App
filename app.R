...
uiTime <- fluidPage(
  ...
  titlePanel("Czas spędzony na korzystaniu z urządzeń"),
...
ui_D <- fluidPage(
  ...
  titlePanel("Czas spędzony każego dnia na danej aplikacji na komputerze oraz na 
                telefonie wyrażony w godzinach"),
  sidebarLayout(
    sidebarPanel(
      radioButtons(
        inputId = "Osoba",
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(
        ...
        #Musiałem ręcznie wybierać i dodawać strony, bo nie ma żadnej konwencji
...
ui_Michal <- fluidPage(
  theme = shinytheme("darkly"),
  titlePanel("Czas spędzony dziennie na poszczególnych stronach
             internetowych oraz zależność długości używania aplikacji
             do listy poszczególnych uruchomień jej"),
  fluidRow(style = "background-color:#393939;",
...
server <- function(input, output) {
  ...
  output$plotTime <- renderPlotly({
    if (is.null(input$name)) {
      df <- as.data.frame(t(c("Wybierz osobę!", 1, 1)))
      p <- ggplot(df) +
      ...
      if (input$type == "Z telefonu") {
        ...
        p <- ggplot(df %>% filter(name %in% input$name)) +
          ...
          labs(title = paste("Czas spędzony na", dev, "dziennie", sep = " "),
               x = "Data",
               y = "Czas w minutach:",
               color = "Osoba:") + 
          theme_gray() +
        ...
        if (input$seq == "Dziennie") {
          ...
          p <- ggplot(df) +
            geom_col(aes(x = hours, y = time, fill = name,
                     ...
                     color = "white", position = "dodge") +
            scale_fill_manual(values = c("navyblue", "#656561", "#a20cad")) +
            labs(title = paste("Czas spędzony na", dev, "godzinowo", sep = " "),
                 x = "Godziny",
                 y = "Czas w minutach",
                 color = "Osoba:") + 
            theme_gray() +
      ...
      if (input$seq2 == "Dziennie") {
        ...
        p <- ggplot(df) +
          geom_col(aes(x = date, y = Odblokowania, fill = name,
                   ...
                   color = "white", position = "dodge") +
          scale_fill_manual(values = c("navyblue", "#656561", "#a20cad")) +
          labs(title = "Suma odblokowań dziennie",
               x = "Data",
               y = "Liczba odblokowań",
               fill = "Osoba:") + 
          theme_gray() +
        ...
        p <- ggplot(df) +
          ...
          labs(title = "Suma odblokowań godzinowo",
               x = "Data",
               y = "Liczba odblokowań",
               fill = "Osoba:") + 
          theme_gray() +
  ...
  output$plot1 <- renderPlotly({
    txt <-  paste("Czas spędzony na komputerze na",input$AppKom1, sep=" ")
  ...
  output$plot2 <- renderPlotly({
    txt <-  paste("Czas spędzony na telefonie na",input$AppTel1, sep=" ")
  ...
  output$plot1_M <- renderPlotly({
    ...
    plot <- ggplot(df, aes(x = date, y = suma, 
      ...
      labs(title = "Czas spędzony dziennie na danej stronie internetowej",
  ...
  output$plot2_M <- renderPlotly({
    ...
    else {df1 <- dftel %>% filter(data.app != "Systemowy menadżer pulpitu")}
    ...
    plot <- plot <- ggplot(df,
                           aes(y = number.of.activations, x = duration, label = data.app, 
                               text = paste0("Aplikacja: ", data.app, "<br>Liczba uruchomień: ", number.of.activations,
                                             "<br>Czas:", round(duration, 3)))) +
      geom_point(colour = "#3ba8fc") +
      labs(title = "Stosunek długości używania alpikacji w danym dniu do 
           ilości uruchomień aplikacji",
           x = "Czas spędzony w aplikacji (w minutach)",
           y = "Ilość uruchomień aplikacji") +
      theme_gray()+
      theme(panel.background = element_rect(fill = '#222222', colour = '#222222'),
            plot.background = element_rect(fill = '#303030', colour = '#303030'))+
      theme(text = element_text(size=10, color = "white"),
  ...
  output$table <- renderTable({
    ...
    if (input$type == "Z komputera") {
      ...
      tab <- t(tab)
      names <- c("", "Całkowity czas korzystania",
                 "Średni dzienny czas korzystania",
                 "Najdłuższe dzienne użytkowanie",
                 "Dzień najdłuższego użytkowania",
                 "Najkrótsze dziennie użytkowanie",
                 "Dzień najkrótszego użytkowania",
                 "Godzina średnio najdłuższego użytkowania",
                 "Średni czas użytkowania w najpopularniejszej godzinie")
      tab <- cbind(names, tab)
      ...
      names <- c("", "Całkowity czas korzystania",
                 ...
                 "Najdłuższe dzienne użytkowanie",
                 "Dzień najdłuższego użytkowania",
                 "Najkrótsze dzienne użytkowanie",
                 "Dzień najkrótszego użytkowania")
      tab <- cbind(names, tab)
